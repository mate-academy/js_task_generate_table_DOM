...
people.forEach((pers) => {
  ...
  const gender = pers.sex === 'm' ? 'Male' : 'Female';
  const age = pers.died - pers.born;
  const century = Math.ceil(pers.died / 100);

  const personData = [pers.name, gender, pers.born, pers.died, age, century];
