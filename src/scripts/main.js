...
for (const { name: personName, sex, born, died } of people) {
  ...
  const data = [personName, sex, born, died];

  data.forEach((pieceOfData) => addCell(pieceOfData, row));
  addCell(died - born, row);
  addCell(Math.ceil(died / 100), row);
